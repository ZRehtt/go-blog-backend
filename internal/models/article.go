...
type Article struct {
	*Model
	...
	Desc     string `json:"desc,omitempty" gorm:"type:varchar(255);comment:文章简述"`
	Content  string `json:"content,omitempty" gorm:"type:longtext;comment:文章内容"`
	CoverURL string `json:"coverURL" gorm:"type:varchar(255);comment:封面图片地址"`
...
func CreateArticle(article *Article) error {
	...
	if err != nil {
		zap.L().Error("Can't create article in db", zap.Any("err", err))
		return err
	}
	return nil
}
...
func GetArticleByID(id int) (*Article, error) {
	...
	if err != nil && err != gorm.ErrRecordNotFound {
		zap.L().Error("Can't find article by ID in db", zap.Any("err", err))
		return nil, err
	}
	return &article, nil
}
...
func GetArticleTotal(page ArticlePage) (int64, error) {
	...
	if err := query.Count(&count).Error; err != nil {
		zap.L().Error("Can't get articles total in db", zap.Any("err", err))
		return 0, err
	}
	return count, nil
}
...
func GetArticlesByPage(pageNumber, pageSize int) ([]*Article, error) {
	...
	if err != nil && err != gorm.ErrRecordNotFound {
		zap.L().Error("Can't get articles by page in db", zap.Any("err", err))
		return nil, err
	}
	return articles, nil
}
...
func UpdateArticle(article Article) error {
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		if err = tx.Updates(&article).Error; err != nil {
			zap.L().Error("Can't update article in db", zap.Any("err", err))
			return err
		}
		return nil
	})
...
func DeleteArticle(id int) error {
	...
	if err := db.Where("id = ? AND deleted_at IS NULL", id).Delete(&Article{}).Error; err != nil {
		zap.L().Error("Can't delete article in db", zap.Any("err", err))
		return err
	}
	return nil
}
